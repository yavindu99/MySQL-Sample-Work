SELECT sag6.name,gname,ou_name,IFNULL(GROUP_CONCAT(jul),'') AS 'jul','July to December' AS 'section',sag6.FB,
IFNULL(GROUP_CONCAT(aug),'') AS 'aug',
IFNULL(GROUP_CONCAT(sep),'') AS 'sep',
IFNULL(GROUP_CONCAT(sag6.oct),'') AS 'oct',
IFNULL(GROUP_CONCAT(nov),'') AS 'nov',
IFNULL(GROUP_CONCAT(sag6.dec),'') AS 'dec'
FROM 
(
	SELECT sag5.name,gname,CASE WHEN gname LIKE 'CHCs' THEN 3 WHEN gname LIKE 'Sub District Hospital' THEN 2 WHEN gname LIKE 'District Hospital' THEN 1 END AS seq,grade,ou_name, CASE WHEN mn=7 THEN grade END AS 'jul',sag5.FB,
	CASE WHEN mn=8 THEN grade END AS 'aug',
	CASE WHEN mn=9 THEN grade END AS 'sep',
	CASE WHEN mn=10 THEN grade END AS 'oct',
	CASE WHEN mn=11 THEN grade END AS 'nov',
	CASE WHEN mn=12 THEN grade END AS 'dec'
	FROM
	(
	...
	(
		...
		FROM
		  ...
		  (
			...
			AND MONTH(p.startdate) BETWEEN 7 AND 12
