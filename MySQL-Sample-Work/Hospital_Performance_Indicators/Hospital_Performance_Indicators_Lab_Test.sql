...
SUM(IFNULL(6071_curr_mn,0)) AS '6071_curr_mn',
SUM(IFNULL(8944_curr_mn,0)) AS '8944_curr_mn',

SUM(IFNULL(6071_las_yr,0)) AS '6071_las_yr',

SUM(IFNULL(6071_bef_2_yr,0)) AS '6071_bef_2_yr'
...
CASE WHEN sag.dataelementid=6071 AND sag.time='cur_mn' THEN sag.value END AS '6071_curr_mn',
CASE WHEN sag.dataelementid=8944 AND sag.time='cur_mn' THEN sag.value END AS '8944_curr_mn',

CASE WHEN sag.dataelementid=6071 AND sag.time='las_yr' THEN sag.value END AS '6071_las_yr',

CASE WHEN sag.dataelementid=6071 AND sag.time='bef_2_yr' THEN sag.value END AS '6071_bef_2_yr'
...
UNION
	...
	FROM datavalue dv
	INNER JOIN period p ON p.periodid=dv.periodid 
	WHERE dv.dataelementid IN (6071)
	AND dv.categoryoptioncomboid IN (1)
	#AND p.startdate BETWEEN '2013-09-01' AND '2013-09-31'
	AND MONTH(p.startdate) = MONTH('2013-11-01')
