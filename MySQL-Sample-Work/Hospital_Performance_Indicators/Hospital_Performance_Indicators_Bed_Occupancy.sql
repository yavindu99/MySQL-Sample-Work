...
SUM(IFNULL(129_curr_mn,0)) AS '129_curr_mn',
SUM(IFNULL(7778_curr_mn,0)) AS '7778_curr_mn',
SUM(IFNULL(8939_curr_mn,0)) AS '8939_curr_mn',

SUM(IFNULL(129_las_yr,0)) AS '129_las_yr',
SUM(IFNULL(7778_las_yr,0)) AS '7778_las_yr',

SUM(IFNULL(129_bef_2_yr,0)) AS '129_bef_2_yr',
SUM(IFNULL(7778_las_yr,0)) AS '7778_bef_2_yr'

FROM
(	
SELECT sag.ou_name,
CASE WHEN sag.dataelementid=8936 AND sag.time='cur_mn' THEN sag.value END AS '8936_curr_mn',
CASE WHEN sag.dataelementid=129 AND sag.time='cur_mn' THEN sag.value END AS '129_curr_mn',
CASE WHEN sag.dataelementid=7778 AND sag.time='cur_mn' THEN sag.value END AS '7778_curr_mn',
CASE WHEN sag.dataelementid=8939 AND sag.time='cur_mn' THEN sag.value END AS '8939_curr_mn',

CASE WHEN sag.dataelementid=129 AND sag.time='las_yr' THEN sag.value END AS '129_las_yr',
CASE WHEN sag.dataelementid=7778 AND sag.time='las_yr' THEN sag.value END AS '7778_las_yr',

CASE WHEN sag.dataelementid=129 AND sag.time='bef_2_yr' THEN sag.value END AS '129_bef_2_yr',
CASE WHEN sag.dataelementid=7778 AND sag.time='las_yr' THEN sag.value END AS '7778_bef_2_yr'
...
(	
	...
	WHERE dv.dataelementid IN (8936,129,7778,8939)
...
UNION
	...
	FROM datavalue dv
	INNER JOIN period p ON p.periodid=dv.periodid 
	WHERE dv.dataelementid IN (129,7778)
	AND dv.categoryoptioncomboid IN (1)
	#AND p.startdate BETWEEN '2013-09-01' AND '2013-09-31'
	AND MONTH(p.startdate) = MONTH('2013-11-01')
