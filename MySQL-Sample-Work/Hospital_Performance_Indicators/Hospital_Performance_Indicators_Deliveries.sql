...
SUM(IFNULL(1254_curr_mn,0)) AS '1254_curr_mn',
SUM(IFNULL(1257_curr_mn,0)) AS '1257_curr_mn',
SUM(IFNULL(8942_curr_mn,0)) AS '8942_curr_mn',

SUM(IFNULL(1254_las_yr,0)) AS '1254_las_yr',
SUM(IFNULL(1257_las_yr,0)) AS '1257_las_yr',

SUM(IFNULL(1254_bef_2_yr,0)) AS '1254_bef_2_yr',
SUM(IFNULL(1257_bef_2_yr,0)) AS '1257_bef_2_yr'

FROM
(	
SELECT sag.ou_name,
CASE WHEN sag.dataelementid=8936 AND sag.time='cur_mn' THEN sag.value END AS '8936_curr_mn',
CASE WHEN sag.dataelementid=1254 AND sag.time='cur_mn' THEN sag.value END AS '1254_curr_mn',
CASE WHEN sag.dataelementid=1257 AND sag.time='cur_mn' THEN sag.value END AS '1257_curr_mn',
CASE WHEN sag.dataelementid=8942 AND sag.time='cur_mn' THEN sag.value END AS '8942_curr_mn',

CASE WHEN sag.dataelementid=1254 AND sag.time='las_yr' THEN sag.value END AS '1254_las_yr',
CASE WHEN sag.dataelementid=1257 AND sag.time='las_yr' THEN sag.value END AS '1257_las_yr',

CASE WHEN sag.dataelementid=1254 AND sag.time='bef_2_yr' THEN sag.value END AS '1254_bef_2_yr',
CASE WHEN sag.dataelementid=1257 AND sag.time='bef_2_yr' THEN sag.value END AS '1257_bef_2_yr'
...
UNION
	...
	FROM datavalue dv
	INNER JOIN period p ON p.periodid=dv.periodid 
	WHERE dv.dataelementid IN (1254,1257)
	AND dv.categoryoptioncomboid IN (1)
	#AND p.startdate BETWEEN '2013-09-01' AND '2013-09-31'
	AND MONTH(p.startdate) = MONTH('2013-11-01')
