#circle - circle
SELECT sag2.Circle,sag2.mon,'100 IFA Tablet' AS 'section', '200 IFA Tablet' AS 'section1','Fully Protected ANC' AS 'section2',
...
FROM
(
	SELECT sag.Circle,sag.District,sag.gname,IFNULL(sag.mon,MONTH('2013-10-01')) AS mon,sag.time,
	...
	(
		...
		LEFT JOIN
			...
			(
			SELECT dv.dataelementid,dv.categoryoptioncomboid,dv.value,dv.sourceid,CASE WHEN MONTH('2013-10-01') BETWEEN 1 AND 3 THEN MONTH('2013-10-01')+9 
				WHEN MONTH('2013-10-01') BETWEEN 4 AND 12 THEN MONTH('2013-10-01')-3 END AS mon,p.startdate
			FROM datavalue dv
		...
		LEFT JOIN
			...
			WHERE dv.dataelementid IN (9,2608,2609)
			AND dv.categoryoptioncomboid IN (1)
			#AND p.startdate BETWEEN '2013-10-01' AND '2012-07-31'
			AND MONTH(p.startdate)=MONTH('2013-10-01')
			AND YEAR(p.startdate)=YEAR('2013-10-01')
			AND periodtypeid = 3
	...
	UNION
		...
		UNION
		 ...
		 #last yr
		SELECT asd.Circle,asd.District,asd.gname,asd.name,'lyr' AS 'time',asd1.mon,
		asd1.dataelementid,asd1.categoryoptioncomboid,asd1.value,MONTH(startdate)
		FROM
		    (
		    SELECT ogm.organisationunitid,og.name AS 'gname',ou.name,os.level,ou1.name AS 'Circle',ou2.name AS 'District'
		...
		SELECT asd.Circle,CASE WHEN asd.gname LIKE 'Corporation Group' THEN asd.name END AS 'District',asd.gname,asd.name,'lyr' AS 'time',asd1.mon,
		asd1.dataelementid,asd1.categoryoptioncomboid,asd1.value,MONTH(startdate)
		FROM
		    (
		    SELECT ogm.organisationunitid,og.name AS 'gname',ou.name,os.level,ou1.name AS 'Circle',ou2.name AS 'District'
